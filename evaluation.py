...
def get_result(processed_data):
    ...
    return avg_acc, avg_execution_rate
...
def evaluate_cot_pred_file(prediction_data, ground_truth_data, client):
    ...
    avg_acc, avg_execution_rate = get_result(output_data)
...
if __name__ == "__main__":
    ...
    ground_truth_filename = args.prediction_path.split("/")[-3].split("-")[0] + ".json"
    ground_truth_file = os.path.join("data", ground_truth_filename)
    ground_truth_raw_data = json.load(open(ground_truth_file, "r"))
    ground_truth_data = {}
    ...
    if os.path.exists(output_path):
        ...
        avg_acc, avg_execution_rate = get_result(processed_data)
    else:
        ...
        outputs, avg_acc, avg_execution_rate = eval_func(prediction_data, ground_truth_data, client)
